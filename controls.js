...
function xayDungBaoCaoLuanGiai(name, degree) {
    ...
    const mucDich = document.getElementById('purpose')?.value || "house";
    // Hỗ trợ cả hai nguồn cấu hình hệ thống toàn cục phòng tránh lỗi crash
    ...
    let birthYearInt = parseInt(yearStr, 10);
    let namAmMệnhChủ = (parseInt(monthStr, 10) < 2 || (parseInt(monthStr, 10) === 2 && parseInt(dayStr, 10) < 4)) ? birthYearInt - 1 : birthYearInt;
    ...
    // =========================================================================
    // 🧠 KHAI THÁC DỮ LIỆU ĐỘNG TỪ LÕI CHÍNH TÔNG - RÚT TRỰC TIẾP TỪ TINHDIEMTONGHOP
    // =========================================================================
    ...
    const thongTin72Hau = tongHop.hauInfo || { ten: "Bình Hòa", chatLuong: "Bình Hòa", diem: 60, ynghia: "Trường khí ổn định.", emoji: "🟡" };

    // TỰ ĐỘNG NỘI SUY HƯỚNG ĐẠI CỤC ĐỂ PHI TINH ĐỒNG BỘ THEO ENGINE LÕI
    const currentSonObj = (typeof PhongThuyCore !== 'undefined' && PhongThuyCore.getSonObj) ? PhongThuyCore.getSonObj(degree) : { huong: "Bắc" };
    const huongToCodeMap = { "Bắc": "N", "Đông Bắc": "NE", "Đông": "E", "Đông Nam": "SE", "Nam": "S", "Tây Nam": "SW", "Tây": "W", "Tây Bắc": "NW" };
    const codeHuongDaiCuc = huongToCodeMap[currentSonObj.huong] || "N";

    // THIẾT LẬP HỆ THỐNG CỬU TINH LƯU NIÊN KHÁCH QUAN ĐỒNG TRỤC
    let saoNienDai = (11 - (namKhaoSat % 9)) % 9;
    if (saoNienDai === 0) saoNienDai = 9;
    const huongToPositionMap = { "N": 1, "SW": 2, "E": 3, "SE": 4, "TrungCung": 5, "NW": 6, "W": 7, "NE": 8, "S": 9 };
    let pos = huongToPositionMap[codeHuongDaiCuc] || 5;
    let saoDaoPhuong = (saoNienDai + (pos - 5) + 9) % 9;
    if (saoDaoPhuong === 0) saoDaoPhuong = 9;

    const danhGiaSaoMap = {
        1: { tenSao: "sao Nhất Bạch Tham Lang đáo phương (Cát)", isHung: false },
        2: { tenSao: "sao Nhị Hắc Cự Môn đáo phương (Bình Hòa/Tiến khí)", isHung: false },
        3: { tenSao: "sao Tam Bích Lộc Tồn đáo phương (Hung Sát)", isHung: true },
        4: { tenSao: "sao Tứ Lục Văn Khúc đáo phương (Suy Khí)", isHung: true },
        5: { tenSao: "sao Ngũ Hoàng Liêm Trinh đáo phương (Đại Hung Sát)", isHung: true },
        6: { tenSao: "sao Lục Bạch Vũ Khúc đáo phương (Cát Tinh)", isHung: false },
        7: { tenSao: "sao Thất Xích Phá Quân đáo phương (Thoái Khí Hung)", isHung: true },
        8: { tenSao: "sao Bát Bạch Tả Phù đóng cung (Trung Tính)", isHung: false },
        9: { tenSao: "sao Cửu Tử Hữu Bật ĐƯƠNG VƯỢNG ĐẠI CÁT", isHung: false }
    };
    const luuNienObj = danhGiaSaoMap[saoDaoPhuong] || { tenSao: "Cửu Tinh Vận Động", isHung: false };
    ...
    // 🎯 ĐỒNG BỘ NHẤT QUÁN MÀU SẮC THEO PH N CẤP KHÍ CỤC CỦA LÕI ĐỘNG
    ...
    if (isKhongVong || tongHop.level === "HUNG CỤC") {
        ...
        mauChu = "#ff3b30";
    } else if (tongHop.level === "BÌNH HOÀ" || luuNienObj.isHung || tongHop.satTinhs.length > 0 || thongTin72Hau.emoji === "🔴" || thongTin72Hau.emoji === "⚠️") {
    ...
    // Định vị màu sắc chuẩn xác cho Hậu mạch ngầm (Sửa lỗi màu xanh của data tĩnh cũ)
    let mauChuHau = "#30d158"; // Mặc định Cát
    if (thongTin72Hau.emoji === "🔴" || thongTin72Hau.emoji === "☠️" || thongTin72Hau.chatLuong.includes("HUNG") || thongTin72Hau.chatLuong.includes("TUYỆT") || thongTin72Hau.chatLuong.includes("TỬ") || thongTin72Hau.chatLuong.includes("K. NÊN")) {
        mauChuHau = "#ff3b30"; // Phạm tử nguyệt chuyển ngay sang đỏ kịch độc
    } else if (thongTin72Hau.emoji === "🟡" || thongTin72Hau.emoji === "⚠️" || thongTin72Hau.chatLuong.includes("BÌNH") || thongTin72Hau.chatLuong.includes("TRUNG") || thongTin72Hau.chatLuong.includes("LỆCH")) {
        mauChuHau = "#ff9f0a"; // Cảnh báo/Bình hòa chuyển sang màu cam
    }

    // Xuất bảng ngày tốt nếu phần gốc không vướng Hung Cục hoặc Không Vong dữ dội
    if (config.isCat) {
        if (ketQuaTrachNhat.length > 0 && !isKhongVong && tongHop.level !== "HUNG CỤC") {
    ...
    // Mật pháp xử lý chuyên sâu lấy giải pháp đồng bộ hóa hoàn toàn từ lõi tích hợp
    ...
    let html = `
        ...
        <div style="font-size: 0.9rem; line-height: 1.7; color: #fff; background: rgba(255,255,255,0.02); padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.06); margin-bottom: 12px;">
            ...
            <div style="margin-bottom: 6px;">🔮 <b>72 Long Khí Xuyên Sơn:</b> <span style="color:#ffd700; font-weight:bold;">Mạch ${thongTin72Hau.ten}</span> — Bản chất mạch ngầm: <b style="color:${mauChuHau};">${thongTin72Hau.chatLuong} (Đạt ${thongTin72Hau.diem}pt)</b></div>
