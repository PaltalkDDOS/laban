...
function generateDirectionsList() {
    ...
    listDirections.forEach(item => {
        ...
        sonGroup.forEach((son, index) => {
            ...
            // Xây dựng bộ lời thoại giải thích thông minh cho từng Sơn vị hẹp
            let thongTinHạnSon = "";
            ...
            if (isCatPurpose) {
                if (score >= 70) {
                    ...
                    solInfo = "Giải pháp: Ưu tiên đặt tâm cửa, đặt thiết bị ngắm đón khí chính xác vào dải độ xanh này để đón trọn tài lộc.";
                } else if (score >= 50) {
                    sonColor = "#dfb76c"; countSonVang++;
                    textInfo = `BÌNH HÒA TRẠCH PHÁP: Sơn vị ${son} đạt trạng thái năng lượng trung tính ổn định (${score}pt), không sinh họa, an toàn cho gia đạo.${thongTinHạnSon}`;
        ...
        let hauInfoTrái = resTrái.hauInfo;
        let hauInfoTâm  = resTâm.hauInfo;
        let hauInfoPhải = resPhải.hauInfo;
        ...
        const getHauStatusAndCount = (hauObj, isCat) => {
            ...
            if (isCat) {
                if (isHungHau) { countHauDo++; return { color: '#ff3b30', label: 'ĐẠI HUNG', desc: 'Mạch khí ngầm dưới lòng đất bị đứt gãy hoặc vướng tạp khí xấu cục bộ. Cực kỳ kỵ dẫn khí vào nhà.' }; }
                if (cl.includes('Cát')) { countHauXanh++; return { color: '#30d158', label: 'CÁT MẠCH', desc: 'Địa mạch long thần thuần khiết vượng khí, gia trì bệ đỡ phong thủy vững chắc lâu dài.' }; }
                countHauVang++; return { color: '#dfb76c', label: 'BÌNH HÒA', desc: 'Địa mạch ngầm giữ trạng thái an định ổn định, an toàn không sinh tai họa.' };
            } else {
                if (isHungHau) { countHauXanh++; return { color: '#30d158', label: 'ĐẮC VỊ', desc: 'Mạch địa khí ngầm mang tính hung sát, rất lý tưởng để đặt công năng xả uế để tiêu trừ tà khí.' }; }
                if (cl.includes('Cát')) { countHauDo++; return { color: '#ff3b30', label: 'K. ĐẶT', desc: 'Long mạch ngầm vượng sạch và thanh khiết, tuyệt đối tránh đặt bể phốt, uế cục đè lên.' }; }
                countHauVang++; return { color: '#dfb76c', label: 'TRUNG TÍNH', desc: 'Địa mạch trung hòa ổn định, có thể cân nhắc đặt công năng kiến trúc phụ.' };
            }
        };
        ...
        let html3HauMạch = `
            <div style="display: flex; gap: 6px; margin-top: 6px; width: 100%; box-sizing: border-box; font-family: sans-serif;">
                <div class="hau-node-item" onclick="showExplanation('${hauInfoTrái.ten.replace(/'/g, "\\'")}', '${getHauExplanation(hauInfoTrái, statusTrái, resTrái).replace(/'/g, "\\'")}', 'Giải pháp: ${hauInfoTrái.giaiphap.replace(/'/g, "\\'")}')"
                ...
                <div class="hau-node-item" onclick="showExplanation('${hauInfoTâm.ten.replace(/'/g, "\\'")}', '${getHauExplanation(hauInfoTâm, statusTâm, resTâm).replace(/'/g, "\\'")}', 'Giải pháp: ${hauInfoTâm.giaiphap.replace(/'/g, "\\'")}')"
                     style="flex: 1; text-align: center; background: rgba(255,255,255,0.04); padding: 6px 4px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.08); cursor: pointer;">
                ...
                <div class="hau-node-item" onclick="showExplanation('${hauInfoPhải.ten.replace(/'/g, "\\'")}', '${getHauExplanation(hauInfoPhải, statusPhải, resPhải).replace(/'/g, "\\'")}', 'Giải pháp: ${hauInfoPhải.giaiphap.replace(/'/g, "\\'")}')"
                     style="flex: 1; text-align: center; background: rgba(255,255,255,0.04); padding: 6px 4px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.08); cursor: pointer;">
        ...
        // ─── ENGINE PHÂN TÍCH BIỆN CHỨNG ĐỒNG BỘ CHO TIÊU ĐỀ/ĐIỂM SỐ ───
        let statusText = "";
        let popExplanation = "";
        let popSolution = "";

        // Tạo chuỗi thông tin sao hạn động bộc lộ nguyên nhân điểm số cho thẻ Direction chính
        let giaiThichSaoHạnCard = "";
        if (item.satTinhs && item.satTinhs.length > 0) {
            let listSao = item.satTinhs.map(s => s.ten).join(", ");
            giaiThichSaoHạnCard = `\\n\\n⚠️ LƯU Ý NIÊN HẠN THỜI GIAN: Điểm số tổng hợp hiện tại là ${item.diemTongHop}pt do phương vị đang vướng phải hung tinh lưu niên năm ${namKhaoSatThucTe} ([${listSao}]) tác động vào phần Ngọn. Đất nền (Gốc) của hướng này vẫn cát lành, qua năm hạn sao dời đi, trường khí sẽ lập tức bùng nượng vượng cát.`;
        } else {
            giaiThichSaoHạnCard = `\\n\\n✨ THIÊN THỜI ĐẮC LỢI: Hướng lớn sạch bóng hoàn toàn đại sát tinh lưu niên trong năm ${namKhaoSatThucTe}, trường khí an định tuyệt đối.`;
        }
        ...
        if (isCatPurpose) {
            if (item.diemTongHop >= 70) {
                ...
                popSolution = `Giải pháp: Gia chủ vẫn hoàn toàn đủ điều kiện quy hoạch lâu dài tại đây. Hãy tiến hành vi chỉnh thiết kế đặt tâm nạp khí lọt khít vào phân độ Sơn vị màu xanh và áp dụng vật phẩm hóa giải để phong tỏa niên hạn.`;
            }
...
function tinhDiemTongHop(cungPhi, degree, namKhảoSát, mucDich, namAm, doRongQuetThucTe) {
    ...
    // Tự động đồng bộ thời gian thực từ hệ thống máy tính nếu để trống
    const namTinhVan = namKhảoSát ? parseInt(namKhảoSát, 10) : new Date().getFullYear();
    let namAmReal = namAm;
    if (!namAmReal) {
        // Nếu không có năm âm, tự động lấy năm khảo sát làm fallback an toàn
        namAmReal = namTinhVan;
    }
    ...
    const cungBátTrạch = bátTrạchMap[cungPhi]?.[codeChuan] || "Phục Vị";
    ...
    // ---------------------------------------------------------------------
    // 📐 BƯỚC 2: TÍNH ĐIỂM NỀN TẢNG (CÁI GỐC CỐ ĐỊNH CỦA ĐẤT VÀ NGƯỜI)
    // ---------------------------------------------------------------------
    const mapDiemBatTrach = {
        "Sinh Khí": 95, "Diên Niên": 90, "Thiên Y": 85, "Phục Vị": 80,
        "Họa Hại": 45, "Lục Sát": 35, "Ngũ Quỷ": 25, "Tuyệt Mệnh": 15
    };
    ...
    // Tính toán Siêu Cấu Trúc Điểm Gốc thiết lập đại cục
    let diemGocTruongKhi = (diemBatTrachGoc * tyTrong.batTrach) + (diemSonGoc * tyTrong.son) + (diemHauGoc * tyTrong.hau);

    // ---------------------------------------------------------------------
    // ⏳ BƯỚC 3: XỬ LÝ PHẦN NGỌN BIẾN THIÊN (THỜI GIAN THIÊN VĂN K_VAN)
    // ---------------------------------------------------------------------
    let heSoVan = 1.0;
    ...
    if (typeof VAN_DATA !== 'undefined' && VAN_DATA[vanSo]?.[codeChuan]) {
        ...
        if (isCatPurpose) {
            heSoVan = (saoNam.loai === "best") ? 1.06 : (saoNam.loai === "worst" ? 0.94 : 1.0);
        } else {
            heSoVan = (saoNam.loai === "worst") ? 1.06 : (saoNam.loai === "best" ? 0.94 : 1.0);
        }
    }

    // Điểm số sau khi chịu tác động dịch chuyển của Thiên Thời Tam Nguyên Cửu Vận
    let diemSauTinhVan = diemGocTruongKhi * heSoVan;

    // ---------------------------------------------------------------------
    // 🛡️ BƯỚC 4: KHẢO SÁT HỆ THỐNG SÁT TINH LƯU NIÊN TRỰC DIỆN (ΣΨ_Sat)
    // ---------------------------------------------------------------------
    const satTinhsKhảoSat = typeof getPhongThuySatTinh === 'function' ? getPhongThuySatTinh(sonName, namTinhVan) : [];
    let tongPhatSatTinh = 0;

    satTinhsKhảoSat.forEach(sat => {
        // Chỉ trừ điểm trực tiếp của Sát tinh tác động lên cung thiết lập Cát
        if (isCatPurpose) {
            if (sat.ten.includes("NGŨ HOÀNG") || sat.ten.includes("THÁI TUẾ")) {
                tongPhatSatTinh += 12; // Mức phạt tối ưu, tránh làm sập điểm cực đoan
            } else {
                tongPhatSatTinh += 6;
            }
        } else {
            // Đối với uế cục cần trấn sát: Gặp ác tinh lại càng gia tăng năng lượng chế sát
            diemSauTinhVan += 4;
        }
    });

    // Tính toán cốt tủy phương thức toán học: PT = [ Gốc * K_Van ] - Sát tinh
    let diemHienThoiQuet = Math.round(diemSauTinhVan - tongPhatSatTinh);

    // ---------------------------------------------------------------------
    // 👁️ BƯỚC 5: BIỆN CHỨNG KHÔNG GIAN MA TRẬN KỀ CẬN (ĐỒNG BỘ NĂM)
    // ---------------------------------------------------------------------
    let sonGroup = [];
    ...
    sonGroup.forEach(s => {
        ...
        if (scGoc >= 70) soSơnTốt++;
        if (scGoc < 50) soSơnXấu++;
    });

    let mốcMạchTrái = Math.round(((normalizedDegree - 5 + 360) % 360) / 5) * 5;
    let mốcMạchPhải = Math.round(((normalizedDegree + 5) % 360) / 5) * 5;
    let hTráiStatic = Data72Hau[mốcMạchTrái.toString()] || { diem: 60, chatLuong: "Bình Hòa" };
    let hPhảiStatic = Data72Hau[mốcMạchPhải.toString()] || { diem: 60, chatLuong: "Bình Hòa" };

    let soHậuTốt = [hTráiStatic.diem, diemHauGoc, hPhảiStatic.diem].filter(d => d >= 70).length;
    let soHậuThựcSựGãy = [hTráiStatic, hauTâmObjStatic, hPhảiStatic].filter(h => h.chatLuong.includes("Hung")).length;

    // ---------------------------------------------------------------------
    // ⚡ BƯỚC 6: HỆ THỐNG PHỦ QUYẾT BIỆN CHỨNG (VETO CHUẨN ĐỜI THỰC)
    // ---------------------------------------------------------------------
    ...
    if (isCatPurpose) {
        if (soHậuThựcSựGãy === 3) {
            HeSoBiệnChứng *= 0.55; // Long mạch đứt gãy thực địa (Tĩnh) -> Hạ điểm nặng
            GhiChuDacBiet = "➔ [CỐT TỦY ĐỊA LONG ĐỨT GÃY]: Địa mạch nền tảng phương vị vướng hung sát cố định, bất lợi trường kỳ.";
        } else if (tongPhatSatTinh > 0) {
            HeSoBiệnChứng *= 0.92; // Chỉ cảnh báo nhẹ bằng hệ số điều tiết vì đây chỉ là hạn theo năm (Động)
            GhiChuDacBiet = `➔ [HẠN TINH ĐÁO PHƯƠNG ${namTinhVan}]: Khí trường nền tốt nhưng vướng sao hạn năm, cần an vị vật phẩm hóa giải để nạp cát khí.`;
        } else if (soSơnTốt === 3 && soHậuTốt >= 2) {
            HeSoBiệnChứng *= 1.08;
            GhiChuDacBiet = "➔ [ĐẮC THỜI CHÂN LONG CAO KHÍ]: Cấu trúc trắc địa vi phân đồng trục cát lành, vượng khí dồi dào.";
        }
    } else {
        if (soSơnXấu >= 2 && soHậuThựcSựGãy >= 2) {
            HeSoBiệnChứng *= 1.10;
            GhiChuDacBiet = "➔ [LONG THẦN KHÓA SÁT HOÀN MỸ]: Toạ hung trấn sát đắc cách, khóa chặt uế khí không cho phát tán.";
        } else if (soSơnTốt >= 2) {
            HeSoBiệnChứng *= 0.70; // Đặt uế cục trúng sơn vượng cát static -> Phạt nặng
            GhiChuDacBiet = "➔ [XẢ UẾ SAI VỊ PHẠM CÁT SƠN]: Đặt uế cục làm ô uế dòng sinh khí sạch của bản trạch.";
        }
    }

    let diemFinal = diemHienThoiQuet * HeSoBiệnChứng;

    // Quét Không Vong bảo vệ ranh giới hình học
    ...
    if (dínhKhôngVongNặng && isCatPurpose) {
        diemFinal = (loaiKhôngVong === "ĐẠI KHÔNG VONG") ? 15 : (diemFinal - 15);
    }

    let diemCuoi = Math.max(5, Math.min(98, Math.round(diemFinal)));
    
    let level = "HUNG CỤC";
    if (diemCuoi >= 82) level = "ĐẠI CÁT"; // Thang điểm thông minh scannable, không ép gắt
    ...
    return {
        diem: diemCuoi, 
        level, 
        message: GhiChuDacBiet || (sonInfoStatic?.luanDoan || "Khí trường bình hòa."), 
        hoaGiai: sonInfoStatic?.hoaGiai || "Giữ không gian sạch sẽ.",
        khongVong: dínhKhôngVongNặng ? { loai: loaiKhôngVong } : null, 
        sonName, 
        hauInfo: {
            ten: hauTâmObjStatic.ten,
            diem: diemHauGoc,
            chatLuong: hauTâmObjStatic.chatLuong,
            emoji: hauTâmObjStatic.chatLuong.includes("Cát") ? "🟢" : (hauTâmObjStatic.chatLuong.includes("Hung") ? "🔴" : "🟡")
        }
    };
}
// ==========================================================================
